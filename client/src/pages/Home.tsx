...
function InternationalProspectingPanel() {
  ...
  const generateOutreach = async () => {
    ...
    try {
      ...
      if (!profile && website && !website.includes("google.com/search") && !website.includes("wikipedia.org/")) { try { profile = await enrich.mutateAsync({ website }); setContactProfile(profile); } catch { profile = null; } }
      ...
      const message = error?.data?.zodError?.fieldErrors?.website?.[0] || error?.data?.message || error?.message || "AI analysis is unavailable. Confirm the built-in AI integration is enabled in Hostinger.";
      setSearchMessage(String(message).replace("Failed to construct URL", "The selected public link is not a valid website URL; AI analysis will continue without website enrichment."));
    }
  };
