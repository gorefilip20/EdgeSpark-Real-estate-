...
function PartnerPage() {
  ...
  const whatsapp = `https://wa.me/2349120045510?text=${encodeURIComponent(`Hello EdgePark Estate, I need help with a ${roleDetails[role].title} partnership application.`)}`;
...
function InternationalProspectingPanel() {
  ...
  const generateOutreach = async () => {
    ...
    try {
      ...
      const candidateWebsite = validHttpUrl(selectedBusiness.website);
      const isResearchUrl = Boolean(candidateWebsite && /(google\.|wikipedia\.|openstreetmap\.|bing\.|yahoo\.|duckduckgo\.)/i.test(new URL(candidateWebsite).hostname));
      const website = candidateWebsite && !isResearchUrl ? candidateWebsite : undefined;
      if (website && !profile?.publicSummary) { try { const enriched = await enrich.mutateAsync({ website }); profile = { ...(profile || {}), ...enriched }; setContactProfile(profile); } catch { /* Keep directory contact fields when enrichment is unavailable. */ } }
      const result = await generateDraft.mutateAsync({ businessName: selectedBusiness.name || "International business", address: selectedBusiness.address, website, phone: profile?.phone || selectedBusiness.phone, contactName: profile?.contactName || undefined, contactRole: profile?.contactRole || "Business Development / Partnerships team", websiteSummary: profile?.publicSummary || undefined, sector: selectedBusiness.types?.join(", "), objective: objective as any, offer, tone: "professional" });
      setDraft(result);
  ...
  return <section className="mt-8 rounded-[1.4rem] border border-[#173b46]/15 bg-[#173b46] p-6 text-white shadow-sm md:p-8">
    ...
    {results.length > 0 && <div className="mt-6"><div className="flex items-center justify-between"><h3 className="font-display text-xl">Contact directory</h3><span className="text-xs text-white/55">{results.length} prospects · contact-first public research</span></div><div className="mt-3 grid gap-3 lg:grid-cols-2">{results.map(business => <article key={business.placeId} className="rounded-2xl bg-white p-5 text-[#173b46]"><div className="flex items-start justify-between gap-3"><div><h4 className="font-semibold">{business.name}</h4><p className="mt-1 text-xs leading-5 text-[#6c7776]">{business.address || "Address unavailable"}</p></div><Badge className="bg-[#d9e4df] text-[#173b46]">{business.countryCode}</Badge></div><div className="mt-4 grid gap-2 text-sm">{business.phone ? <a href={`tel:${business.phone}`} className="font-semibold text-[#bd7b4b]">{business.phone}</a> : <span className="text-[#8a918e]">Phone not listed</span>}{business.email ? <a href={`mailto:${business.email}`} className="truncate text-[#bd7b4b]">{business.email}</a> : <span className="text-[#8a918e]">Email not publicly listed</span>}{business.contactStatus ? <span className="text-xs text-[#6c7776]">{business.contactStatus}</span> : null}{validHttpUrl(business.website) ? <a href={validHttpUrl(business.website)} target="_blank" rel="noreferrer" className="truncate text-[#173b46] underline">{business.website}</a> : <span className="text-[#8a918e]">Website not listed</span>}</div><div className="mt-4 flex flex-wrap gap-2"><Button type="button" size="sm" onClick={() => { selectBusiness(business); setDetailOpen(true); save.mutate({ placeId: business.placeId, region: business.region, countryCode: business.countryCode }); }} className="rounded-full bg-[#173b46] text-white">Open full profile</Button><a href={pitchLink(business)} className="inline-flex h-9 items-center rounded-full border border-[#173b46]/15 px-3 text-xs font-semibold">Prepare email <Mail className="ml-2 h-3.5 w-3.5" /></a>{validHttpUrl(business.mapsUrl) && <a href={validHttpUrl(business.mapsUrl)} target="_blank" rel="noreferrer" className="inline-flex h-9 items-center rounded-full border border-[#173b46]/15 px-3 text-xs font-semibold">Google Maps <ArrowUpRight className="ml-2 h-3.5 w-3.5" /></a>}</div></article>)}</div><p className="mt-3 text-[11px] text-white/45">Contact details are shown only when publicly listed. Verify the source before outreach and follow applicable privacy, marketing, and platform policies.</p></div>}
    ...
    <div className="mt-8 border-t border-white/15 pt-6"><div className="flex items-center justify-between"><h3 className="font-display text-xl">Saved outreach pipeline</h3><span className="text-xs text-white/55">{saved.length} saved</span></div>{saved.length ? <div className="mt-3 grid gap-3 lg:grid-cols-2">{saved.map((prospect: any) => <div key={prospect.id} className="rounded-2xl bg-white/10 p-4"><div className="flex items-center justify-between gap-3"><div><div className="font-semibold">Place ID {prospect.placeId}</div><div className="mt-1 text-xs text-white/55">{prospect.region} · {prospect.countryCode}</div></div><select value={prospect.status} onChange={e => update.mutate({ id: prospect.id, status: e.target.value as any })} className="h-9 rounded-md border-0 bg-white px-2 text-xs text-[#173b46]"><option value="new">New</option><option value="researching">Researching</option><option value="contacted">Contacted</option><option value="meeting">Meeting</option><option value="won">Won</option><option value="archived">Archived</option></select></div><Textarea defaultValue={prospect.notes || ""} placeholder="Add research notes…" className="mt-3 min-h-16 border-white/15 bg-white/10 text-white placeholder:text-white/40" onBlur={e => { if (e.target.value !== (prospect.notes || "")) update.mutate({ id: prospect.id, notes: e.target.value }); }} /></div>)}</div> : <p className="mt-3 text-sm text-white/55">Save a result to begin tracking research, outreach, meetings, and partnership outcomes.</p>}</div>
      {detailOpen && selectedBusiness && <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#173b46]/70 p-4" role="dialog" aria-modal="true" aria-label={`${selectedBusiness.name} full profile`}><div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-[1.5rem] bg-white text-[#173b46] shadow-2xl"><div className="sticky top-0 flex items-start justify-between gap-4 border-b border-[#deded5] bg-white p-5"><div><div className="eyebrow">Partner / listing detail</div><h3 className="mt-1 font-display text-3xl">{selectedBusiness.name}</h3><p className="mt-1 text-sm text-[#6c7776]">{selectedBusiness.address || selectedBusiness.countryCode || "Selected market"}</p></div><Button type="button" onClick={() => setDetailOpen(false)} className="rounded-full bg-[#173b46] text-white">Close</Button></div><div className="grid gap-5 p-5 lg:grid-cols-[1.15fr_.85fr]"><div className="space-y-5"><div className="grid gap-3 sm:grid-cols-3"><div className="rounded-xl bg-[#f8f7f3] p-4"><div className="text-[10px] uppercase tracking-[0.14em] text-[#6c7776]">Source</div><div className="mt-2 font-semibold">{selectedBusiness.source || "Public directory"}</div></div><div className="rounded-xl bg-[#f8f7f3] p-4"><div className="text-[10px] uppercase tracking-[0.14em] text-[#6c7776]">Category</div><div className="mt-2 font-semibold">{selectedBusiness.category || selectedBusiness.types?.join(", ") || "Business"}</div></div><div className="rounded-xl bg-[#f8f7f3] p-4"><div className="text-[10px] uppercase tracking-[0.14em] text-[#6c7776]">Country</div><div className="mt-2 font-semibold">{selectedBusiness.countryCode || "International"}</div></div></div><div className="rounded-xl border border-[#deded5] p-4"><div className="eyebrow">Location metrics</div><div className="mt-3 grid gap-3 sm:grid-cols-3 text-sm"><div><span className="font-semibold">Latitude:</span> {selectedBusiness.latitude ?? selectedBusiness.lat ?? "Not listed"}</div><div><span className="font-semibold">Longitude:</span> {selectedBusiness.longitude ?? selectedBusiness.lng ?? "Not listed"}</div><div><span className="font-semibold">Rating:</span> {selectedBusiness.rating ?? "Not listed"}</div><div><span className="font-semibold">Reviews:</span> {selectedBusiness.userRatingsTotal ?? selectedBusiness.reviewCount ?? "Not listed"}</div><div><span className="font-semibold">Types:</span> {selectedBusiness.types?.join(", ") || "Not listed"}</div><div><span className="font-semibold">ID:</span> {selectedBusiness.placeId || "Not listed"}</div></div></div><div className="rounded-xl border border-[#deded5] p-4"><div className="eyebrow">Public contact intelligence</div><div className="mt-3 grid gap-2 text-sm"><div><span className="font-semibold">Decision-maker:</span> {contactProfile?.contactName || "Not publicly listed"}</div><div><span className="font-semibold">Route:</span> {contactProfile?.contactRole || "Business Development / Partnerships team"}</div><div><span className="font-semibold">Email:</span> {contactProfile?.email ? <a className="text-[#bd7b4b] underline" href={`mailto:${contactProfile.email}`}>{contactProfile.email}</a> : "Not publicly listed"}</div><div><span className="font-semibold">Phone:</span> {contactProfile?.phone || selectedBusiness.phone ? <a className="text-[#bd7b4b] underline" href={`tel:${contactProfile?.phone || selectedBusiness.phone}`}>{contactProfile?.phone || selectedBusiness.phone}</a> : "Not publicly listed"}</div><div><span className="font-semibold">Address:</span> {selectedBusiness.address || "Not listed"}</div></div><Button type="button" size="sm" onClick={() => discoverContacts(selectedBusiness)} disabled={discover.isPending} className="mt-4 rounded-full bg-[#e8efe9] text-[#173b46]">{discover.isPending ? "Checking public sources…" : "Refresh public contacts"}</Button></div></div><div className="space-y-5"><div className="rounded-xl bg-[#173b46] p-5 text-white"><div className="eyebrow text-white/60">Next actions</div><p className="mt-2 text-sm leading-6 text-white/75">Use verified public channels only. Confirm the organisation and the right decision-maker before sending a proposal.</p><div className="mt-4 grid gap-2">{contactProfile?.email && <a className="rounded-full bg-[#d59462] px-4 py-2 text-center text-sm font-semibold text-white" href={`mailto:${contactProfile.email}`}>Email public contact</a>}{(contactProfile?.phone || selectedBusiness.phone) && <a className="rounded-full border border-white/20 px-4 py-2 text-center text-sm font-semibold text-white" href={`tel:${contactProfile?.phone || selectedBusiness.phone}`}>Call public number</a>}<Button type="button" onClick={() => { setDetailOpen(false); setSearchMessage("The AI outreach workspace is ready below the directory."); }} className="rounded-full bg-white px-4 py-2 text-sm font-semibold text-[#173b46]">Open outreach workspace</Button></div></div><div className="rounded-xl border border-[#deded5] p-4"><div className="eyebrow">Official sources</div><div className="mt-3 grid gap-2 text-sm">{validHttpUrl(selectedBusiness.website) && <a className="truncate text-[#bd7b4b] underline" href={validHttpUrl(selectedBusiness.website)} target="_blank" rel="noreferrer">Open website</a>}{validHttpUrl(selectedBusiness.mapsUrl) && <a className="truncate text-[#bd7b4b] underline" href={validHttpUrl(selectedBusiness.mapsUrl)} target="_blank" rel="noreferrer">Open map / directory source</a>}{contactProfile?.sourceUrl && <a className="truncate text-[#bd7b4b] underline" href={contactProfile.sourceUrl} target="_blank" rel="noreferrer">Open contact source</a>}{!validHttpUrl(selectedBusiness.website) && !validHttpUrl(selectedBusiness.mapsUrl) && !contactProfile?.sourceUrl && <span className="text-[#8a918e]">No source URL was returned.</span>}</div></div></div></div></div></div>}
  </section>;
}
function AdminPage() {
  const { user, loading } = useAuth();
