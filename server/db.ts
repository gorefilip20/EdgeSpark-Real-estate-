...
export async function ensureAuthTables(db: any) { try { await db.execute(sql.raw("CREATE TABLE IF NOT EXISTS localUsers (id INT AUTO_INCREMENT PRIMARY KEY, openId VARCHAR(64) NOT NULL UNIQUE, name TEXT NULL, email VARCHAR(320) NOT NULL UNIQUE, loginMethod VARCHAR(64) NOT NULL DEFAULT 'email', role ENUM('user','admin') NOT NULL DEFAULT 'user', createdAt TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, updatedAt TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, lastSignedIn TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP)")); await db.execute(sql.raw("CREATE TABLE IF NOT EXISTS localAccounts (id INT AUTO_INCREMENT PRIMARY KEY, userId INT NOT NULL UNIQUE, passwordHash VARCHAR(255) NOT NULL, createdAt TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, updatedAt TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP)")); } catch (error) { console.error("[Database] Authentication tables are unavailable:", error instanceof Error ? error.message : error); throw new Error("AUTH_SCHEMA_NOT_READY"); } }
